...
def process_mobility_value(cell):
    """ Process mobility values of any type. """
    if pd.isna(cell):
        return 0.0

    # If cell is a string, strip whitespace and check for known values.
    if isinstance(cell, str):
        ...
        if cell == "+":
            ...
            # Try converting to float if it's not a recognized string.
            try:
                f_val = float(cell)
                return f_val
            except ValueError:
    ...
    # For non-string values, try to convert to float.
    try:
        f_val = float(cell)
    except (ValueError, TypeError):
        return 0.0

    # Check for known numeric values.
    if f_val == 1:
        return 1.0
    elif f_val == 2:
        return 2.0
...
def process_tooth_integer_values(cell):
    ...
    """
    s = str(cell).strip()
    # Check if the cell is missing, empty, equals 'F', or consists entirely of 'F' tokens (e.g., "F  F  F")
    if cell is pd.NA or s == '' or all(token == 'F' for token in s.split()):
        return ["Missing"] * 3

    if len(cell) == 1 or (len(s) == 2 and s == cell):
        return ["Missing", "Missing", s]
